...
const getStatusBadge = (order: Order, t: any) => {
  if (order.status === "cancelled") {
    return (
      <Badge className="bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 border border-red-200 dark:border-red-800 px-3 py-1">
        ...
        {t("profile.farmer.orders.status.cancelled")}
      </Badge>
    );
  }
  ...
  if (order.isDelivered) {
    return (
      <Badge className="bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800 px-3 py-1">
        ...
        {t("profile.farmer.orders.status.delivered")}
      </Badge>
    );
  }
  ...
  if (order.isOutForDelivery) {
    return (
      <Badge className="bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800 px-3 py-1">
        ...
        {t("profile.farmer.orders.status.outForDelivery")}
      </Badge>
    );
  }
  ...
  if (order.status === "confirmed") {
    return (
      <Badge className="bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-800 px-3 py-1">
        <CheckCircle className="h-3.5 w-3.5 mr-1.5" />
        {t("profile.farmer.orders.status.confirmed")}
      </Badge>
    );
  }
  ...
  return (
    <Badge className="bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-800 px-3 py-1">
      ...
      {t("profile.farmer.orders.status.pending")}
    </Badge>
  );
};
  ...
  if (mode === "DELIVERYBYFARMER") {
    return (
      ...
      >
        ...
        {t("profile.farmer.orders.delivery.deliveryByYou")}
      </Badge>
    );
  }
  return (
    ...
    >
      ...
      {t("profile.farmer.orders.delivery.pickupByBuyer")}
    </Badge>
  );
};
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        toast.error(t("profile.farmer.orders.toasts.loadFailed"));
        setLoading(false);
      }
    };
  ...
  const changeOrderStatus = async (
    ...
    try {
      const res = await axios.put("/api/order/changeStatus", { id, status });
        ...
        toast.success(
          t("profile.farmer.orders.toasts.statusUpdated", {
            status: t(`profile.farmer.orders.status.${status}`),
          })
        );
        sendNotification({
          userId: buyerId.replace("buy_", "user_"),
          title: t("profile.farmer.orders.notification.statusTitle", {
            status: t(`profile.farmer.orders.status.${status}`),
          }),
          message: t("profile.farmer.orders.notification.statusMessage", {
            farmer: farmerName,
            status: t(`profile.farmer.orders.status.${status}`),
          }),
          type: "order",
      ...
      toast.error(t("profile.farmer.orders.toasts.statusUpdateFailed"));
    }
  };
  ...
  const setOutForDelivery = async (
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        toast.success(t("profile.farmer.orders.toasts.outForDelivery"));
        sendNotification({
          userId: buyerId.replace("buy_", "user_"),
          title: t("profile.farmer.orders.notification.outForDeliveryTitle"),
          message: t("profile.farmer.orders.notification.outForDeliveryMessage", {
            farmer: farmerName,
          }),
          type: "Order",
        });
      ...
      toast.error(t("profile.farmer.orders.toasts.outForDeliveryFailed"));
    }
  };
  ...
  const conformDelivery = async (
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        toast.success(t("profile.farmer.orders.toasts.delivered"));
        sendNotification({
          userId: buyerId.replace("buy_", "user_"),
          title: t("profile.farmer.orders.notification.deliveredTitle"),
          message: t("profile.farmer.orders.notification.deliveredMessage", {
            farmer: farmerName,
          }),
          type: "Order",
        });
      ...
      toast.error(t("profile.farmer.orders.toasts.deliveredFailed"));
    }
  };
    ...
    <main className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800">
      <div className="max-w-7xl mx-auto p-4 md:p-8 space-y-8">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                      <h1 className="text-3xl md:text-4xl font-bold text-gray-900 dark:text-white">
                        {t("profile.farmer.orders.title")}
                      </h1>
                      <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                        {t("profile.farmer.orders.subtitle")}
                      </p>
                </div>
              </div>
            </div>
            ...
            <Link href="/profile/farmer/negotiations">
              <Button className="bg-blue-600 hover:bg-blue-700 dark:bg-blue-700 dark:hover:bg-blue-600 text-white shadow-lg hover:shadow-xl transition-all hover:scale-105 h-11 px-6">
                <MessageCircle className="h-4 w-4 mr-2" />
                    {t("profile.farmer.orders.actions.negotiations")}
        ...
        {!loading && orders.length > 0 && (
          ...
          >
            <StatCard
                label={t("profile.farmer.orders.stats.total")}
              value={stats.total}
              icon={
            ...
            />
            <StatCard
                label={t("profile.farmer.orders.stats.pending")}
              value={stats.pending}
              icon={
            ...
            />
            <StatCard
                label={t("profile.farmer.orders.stats.confirmed")}
            ...
            />
            <StatCard
                label={t("profile.farmer.orders.stats.delivered")}
        ...
        >
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="flex-1 relative group">
              ...
              <input
                ...
                placeholder={t("profile.farmer.orders.searchPlaceholder")}
            ...
            <div className="flex gap-2">
              <div className="relative">
                ...
                >
                  <option value="all">{t("profile.farmer.orders.filters.all")}</option>
                  <option value="pending">{t("profile.farmer.orders.filters.pending")}</option>
                  <option value="confirmed">{t("profile.farmer.orders.filters.confirmed")}</option>
                  <option value="delivered">{t("profile.farmer.orders.filters.delivered")}</option>
                  <option value="cancelled">{t("profile.farmer.orders.filters.cancelled")}</option>
                </select>
              </div>
            </div>
          </div>
          ...
          {search && filteredOrders.length > 0 && (
            <p className="text-xs text-gray-500 dark:text-gray-400">
              {t("profile.farmer.orders.searchResults", { count: filteredOrders.length })}
            </p>
          )}
        </div>
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-24">
            ...
            <p className="text-gray-600 dark:text-gray-400 font-medium">
              {t("profile.farmer.orders.loading")}
            </p>
          </div>
        )}
            ...
            <div className="text-center space-y-4">
              ...
              <div>
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">
                  {t("profile.farmer.orders.empty.title")}
                </h2>
                <p className="text-gray-600 dark:text-gray-400 max-w-md mx-auto">
                  {t("profile.farmer.orders.empty.body")}
                </p>
              </div>
            </div>
          </div>
            ...
            {filteredOrders.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 rounded-xl border-2 border-dashed border-gray-300 dark:border-gray-700">
                ...
                <p className="text-gray-500 dark:text-gray-400 font-medium">
                  {t("profile.farmer.orders.noResults")}
                </p>
              </div>
            ) : (
                ...
                >
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 text-sm text-gray-600 dark:text-gray-400">
                          <div className="flex items-center gap-2 bg-gray-50 dark:bg-gray-700/30 px-3 py-1.5 rounded-full w-fit">
                            <span className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                              {t("profile.farmer.orders.labels.buyer")}
                            </span>
                  ...
                  <CardContent className="space-y-4">
                        ...
                        >
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                              {item.quantity} × {item.unit} @ {t(
                                "profile.farmer.orders.currency"
                              )}
                          ...
                          <div className="text-right ml-3 flex-shrink-0">
                            <span className="font-semibold text-gray-900 dark:text-white text-sm">
                              {t("profile.farmer.orders.currency")}
                    ...
                    <div className="flex justify-between items-center p-3.5 rounded-lg bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 border border-amber-200 dark:border-amber-800/50">
                      <span className="font-semibold text-gray-900 dark:text-white text-sm">
                        {t("profile.farmer.orders.totalEarning")}
                      </span>
                    ...
                    {order.hasPayment && (
                      <div className="p-2.5 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg border border-emerald-200 dark:border-emerald-800/50 flex items-center gap-2.5">
                        ...
                        <p className="text-xs text-emerald-700 dark:text-emerald-300 font-medium">
                          {t("profile.farmer.orders.paymentVerified")}
                        </p>
                      </div>
                    )}
                    ...
                    <div className="flex flex-wrap items-center gap-2 pt-3 border-t border-gray-200 dark:border-gray-700">
                      ...
                      >
                        ...
                        >
                          ...
                          {t("profile.farmer.orders.actions.view")}
                        </Button>
                      </Link>
                          ...
                          <>
                            {order.status !== "confirmed" && (
                              ...
                              >
                                ...
                                {t("profile.farmer.orders.actions.accept")}
                              </Button>
                            )}
                            {!order.hasPayment && (
                              <Button
                              ...
                              >
                                ...
                                {t("profile.farmer.orders.actions.reject")}
                              </Button>
                            )}
                          </>
                        )}
                        ...
                        !order.isOutForDelivery && (
                          ...
                          >
                            <Truck className="h-4 w-4 mr-1.5" />
                              {t("profile.farmer.orders.actions.outForDelivery")}
                          </Button>
                        )}
                      ...
                      {order.deliveryMode === "DELIVERYBYFARMER" &&
                        ...
                        !order.isDelivered && (
                          ...
                          >
                            <CheckCircle className="h-4 w-4 mr-1.5" />
                            {t("profile.farmer.orders.actions.delivered")}
                          </Button>
                        )}
                        ...
                        order.status === "confirmed" && (
                          <Badge className="bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-800">
                            <Clock className="h-3 w-3 mr-1" />
                              {t("profile.farmer.orders.badges.waitingPickup")}
                          </Badge>
                        )}
                      ...
                      {order.isDelivered && (
                        <Badge className="bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800">
                          ...
                          {t("profile.farmer.orders.badges.complete")}
                        </Badge>
                      )}
                      ...
                      {order.status === "cancelled" && (
                        <Badge className="bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 border border-red-200 dark:border-red-800">
                          ...
                          {t("profile.farmer.orders.badges.cancelled")}
                        </Badge>
                      )}
                    </div>
                  </CardContent>
